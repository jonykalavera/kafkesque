...
@click.pass_context
@click.option("--url", "-u", click.STRING, help="Webhook URL", required=True)
@click.option(
    "--topic",
    "-t",
    click.STRING,  # Should we allow to subscribe to multiple topics? In that case it should be "--topics".
...
@main.command("delete")
@click.pass_context
@click.option("--id", "-i", click.INT, help="Webhook id")
...
@main.command("ls")
@click.pass_context
@click.option("--long", "-l", is_flag=True, help="Show in long format")
@click.option("--active", "-a", is_flag=True, help="Show only active webhooks")
